...
<?php else : ?>
    ...
    <div class="col-xs-12">
        <div class="box box-solid">
            <div class="box-header with-border">
                ...
                <h3 class="box-title pull-right">Status Bayar : <?= $data['statusText'] ?></h3>
            </div>
            <?= form_open('penerimaan/update', ['id' => 'form_create'], ['idterima' => $data['idterima'], 'idminta' => $data['dataRequest']['idminta']]) ?>
            <div class="box-body">
                <div class="row">
                    <div class="col-md-6">
                            ...
                            <div class="col-lg-3 col-sm-6 col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group date">
                                        ...
                                        <input type="text" name="tanggal" class="form-control pull-right datepicker" placeholder="dd-mm-yyyy" value="<?= $data['tanggalIndo'] ?>">
                                    </div>
                                    <div id="tanggal"></div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="pemasok" data-placeholder="Pilih Pemasok" style="width: 100%;">
                                        ...
                                        <?php foreach ($supplier as $s) { ?>
                                            <option value="<?= $s['id_supplier'] ?>" <?= $s['id_supplier'] == $data['idpemasok'] ? 'selected' : '' ?>><?= $s['nama_supplier'] ?></option>
                                        <?php } ?>
                                    </select>
                                    <div id="pemasok"></div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="gudang" data-placeholder="Pilih Gudang" style="width: 100%;">
                                        ...
                                        <?php foreach ($gudang as $g) { ?>
                                            <option value="<?= $g['id_gudang'] ?>" <?= $data['idgudang'] == $g['id_gudang'] ? 'selected' : null ?>><?= $g['nama_gudang'] ?></option>
                                        <?php } ?>
                                    </select>
                                    <div id="gudang"></div>
                                </div>
    ...
    <script>
        ...
        var idminta = $('input[name=\'idminta\']').val();
        ...
        function data_request() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if (resp.status == "0100") {
                        html = '<h5 class="data-item title" style="margin-top: 0; font-weight: 600">Data permintaan barang yang telah dipilih</h5>';
        ...
        function data_tmp() {
            ...
            $.ajax({
                url: BASE_URL + 'penerimaan/tmp-edit/data',
                method: 'GET',
                dataType: 'json',
                data: {
                    idterima: idterima
                ...
                success: function(resp) {
                    var html = '';
                    var no = 1;
                    for (var i = 0; i < resp['dataProduk'].length; i++) {
                        html += '<tr>';
                        ...
                        html += '&nbsp;';
                        html += '<a href="javascript:void(0)" class="destroy" id="' + resp['dataProduk'][i]['iddetailterima'] + '"><i class="icon-trash text-red"></i></a>';
                        html += '</td>';
                        html += '</tr>';
                        no++;
                    }
                    ...
                    html += '<th class="text-right">' + resp['totalFormat'] + '</th>';
                    html += '</tr>';
                    $('#data_tmp').html(html);
                }
            });
        ...
        $(document).on('click', '.create', function() {
            ...
            $.ajax({
                url: BASE_URL + 'penerimaan/tmp-edit/create',
                type: 'GET',
                data: {
                    idterima: idterima,
                    iddetail: iddetail
                ...
                success: function(resp) {
                    if ((resp == '0101')) {
                        toastr.error('Produk sedang diinputkan oleh user lain.');
                    } else if ((resp == '0102')) {
        ...
        $(document).on('click', '.edit', function() {
            var id = $(this).attr('id');
            $.ajax({
                url: BASE_URL + 'penerimaan/tmp-edit/edit',
                type: 'GET',
                data: {
                    id: id
                },
        ...
        $(document).on('click', '.destroy', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: BASE_URL + 'penerimaan/tmp-edit/destroy',
                        type: 'GET',
                        dataType: 'json',
                        data: {
                            id: id
